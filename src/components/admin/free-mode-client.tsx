...
import type { Settings, FreeServiceSetting } from "@/lib/db/types";
...
export function FreeModeClient() {
  ...
  const handleAddFreeService = async () => {
    ...
    const updatedFreeServices = [...(initialSettings.freeServices || []), newFreeService];
  ...
  const handleDeleteFreeService = async (id: string) => {
    const updatedFreeServices = (initialSettings.freeServices || []).filter(fs => fs.id !== id);
    setIsSaving(true);
    try {
      await updateSettings({ freeServices: updatedFreeServices });
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div className="flex items-center space-x-4 rounded-md border p-4">
               <Switch
                checked={initialSettings.isFreeModeActive || false}
            ...
            <CardContent>
                <div className="border rounded-lg">
                    <Table>
                        ...
                        <TableBody>
                            {(initialSettings.freeServices || []).length > 0 ? (
                                initialSettings.freeServices?.map(fs => (
                                    <TableRow key={fs.id}>
