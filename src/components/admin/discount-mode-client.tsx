...
import type { Settings, DiscountSetting } from "@/lib/db/types";
...
export function DiscountModeClient() {
  const { brands, models, services, settings: initialSettings, refreshData } = useAdminData();
  ...
  const handleActiveChange = async (isActive: boolean) => {
    setIsSaving(true);
    try {
        await updateSettings({ isDiscountModeActive: isActive });
        toast({
            title: "Éxito",
  ...
  const handleOpenModal = (discount: DiscountSetting | null) => {
    if (discount) {
        ...
        const brand = brands.find(b => b.name === discount.brandName);
        setSelectedBrandId(brand?.id || "clear");

        setTimeout(() => {
            if (discount.modelName) {
                const model = models.find(m => m.name === discount.modelName && m.brand === discount.brandName);
  ...
  const handleAddOrUpdateDiscount = async () => {
    ...
    setIsSaving(true);
    try {
      await updateSettings({ discounts: updatedDiscounts });
  ...
  const getDiscountDisplayName = (discount: DiscountSetting): string => {
    ...
    if (discount.serviceId) {
        return services.find(s => s.id === discount.serviceId)?.name || "Servicio desconocido";
    }
  ...
  const applicableServices = useMemo(() => {
    ...
    return getApplicableServices(selectedModel, services);
