...
import { PlusCircle, MoreHorizontal, Trash, Edit, Loader2, Bold, Italic, Underline, AlignLeft, AlignCenter, AlignRight, Eraser, X, Palette, Copy, Pilcrow, CaseUpper, CaseLower, TextQuote, Type, Text, Link2, Clipboard, ClipboardPaste } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const RichTextField = ({ id, value, onChange, placeholder, onCopyFormat, onPasteFormat, copiedFormat }: { id: string, value: string, onChange: (value: string) => void, placeholder?: string, onCopyFormat: () => void, onPasteFormat: () => void, copiedFormat: string | null }) => {
    ...
    return (
      <div className="rounded-md border border-input">
        <div className="flex items-center gap-1 p-1 border-b flex-wrap">
          ...
          <Button type="button" variant="ghost" size="icon" onMouseDown={(e) => { e.preventDefault(); onCopyFormat(); }} title="Copiar Formato">
            <Clipboard className="h-4 w-4" />
          </Button>
          <Button type="button" variant="ghost" size="icon" onMouseDown={(e) => { e.preventDefault(); onPasteFormat(); }} title="Pegar Formato" disabled={!copiedFormat}>
...
export function PopupsClient({ initialPopups, initialServices, initialBrands }: PopupsClientProps) {
  ...
  const handleCopyFormat = () => {
    ...
    if (!selection || selection.rangeCount === 0 || selection.isCollapsed) {
        toast({ variant: "destructive", title: "Error", description: "Selecciona un texto para copiar su formato." });
        return;
    }
    const range = selection.getRangeAt(0);
    let parent = range.startContainer.parentElement;

    // We're looking for a SPAN with inline styles
    while (parent && (parent.tagName !== 'SPAN' || !parent.getAttribute('style'))) {
        if (parent.contentEditable === 'true') { // Stop at the editor boundary
            parent = null;
            break;
        }
    ...
    if (parent) {
        setCopiedFormat(parent.getAttribute('style'));
        toast({ title: "Éxito", description: "Formato copiado." });
    } else {
        toast({ variant: "destructive", title: "Error", description: "El texto seleccionado no tiene un formato especial para copiar." });
    }
  };

  const handlePasteFormat = () => {
      if (!copiedFormat) {
          toast({ variant: "destructive", title: "Error", description: "Primero copia un formato." });
          return;
      }
      const selection = window.getSelection();
      if (!selection || selection.rangeCount === 0 || selection.isCollapsed) {
          toast({ variant: "destructive", title: "Error", description: "Selecciona el texto donde quieres pegar el formato." });
          return;
      }
      ...
      if (selectedText) {
          const span = document.createElement('span');
          span.setAttribute('style', copiedFormat);
          range.surroundContents(span);
      }
      
      selection.removeAllRanges();
      
      // Manually trigger the onInput for all editors to update their state
      document.getElementById('title-editor')?.dispatchEvent(new Event('input', { bubbles: true }));
      descriptions.forEach((_, index) => {
        document.getElementById(`description-editor-${index}`)?.dispatchEvent(new Event('input', { bubbles: true }));
      });
  };
  ...
  const cleanupHtml = (html: string) => {
    ...
    tempDiv.querySelectorAll('span:not([style]), span[style=""]').forEach(span => {
      ...
      if (!span.hasAttributes() || (span.attributes.length === 1 && span.hasAttribute('style') && htmlSpan.style.length === 0)) {
  ...
  const handleSave = async () => {
    ...
    const popupData = {
      ...
      mediaType: mediaUrl ? ('youtube' as const) : undefined,
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-4 max-h-[70vh] overflow-y-auto pr-4">
              ...
              <div className="md:col-span-2 space-y-4">
                <div className="grid gap-2">
                    ...
                    <RichTextField id="title-editor" value={title} onChange={setTitle} placeholder="Escribe el título aquí..." onCopyFormat={handleCopyFormat} onPasteFormat={handlePasteFormat} copiedFormat={copiedFormat} />
                </div>
                ...
                {descriptions.map((desc, index) => (
                  <div key={index} className="grid gap-2">
                    ...
                    <RichTextField id={`description-editor-${index}`} value={desc} onChange={(val) => handleDescriptionChange(index, val)} placeholder={`Párrafo de descripción ${index + 1}`} onCopyFormat={handleCopyFormat} onPasteFormat={handlePasteFormat} copiedFormat={copiedFormat} />
                  </div>
                ))}
