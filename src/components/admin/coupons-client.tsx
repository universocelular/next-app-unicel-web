...
export function CouponsClient() {
  const { brands, models, services, coupons, refreshData } = useAdminData();
  ...
  const handleEdit = (coupon: Coupon) => {
    ...
    const brand = brands.find(b => b.name === coupon.brandName);
    const model = models.find(m => m.id === coupon.modelId);
  ...
  const getCouponDisplayName = (coupon: Coupon): React.ReactNode => {
    const model = coupon.modelId ? models.find(m => m.id === coupon.modelId) : null;
  ...
  const applicableServices = useMemo(() => {
    ...
    return [];
  }, [selectedModel, selectedBrandId, services]);
